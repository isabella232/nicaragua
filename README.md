...
```
cd nicaragua
mkvirtualenv --no-site-packages nicaragua
...
```
* * * * * ubuntu bash /home/ubuntu/apps/nicaragua/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|nicaragua|tweet|`location`||||
|nicaragua|facebook|`location`||||
|nicaragua|email|`location`||||
|nicaragua|new-comment||||
|nicaragua|open-share-discuss||||
|nicaragua|close-share-discuss||||
|nicaragua|summary-copied||||
|nicaragua|featured-tweet-action|`action`||``tweet_url``|
|nicaragua|featured-facebook-action|`action`||``post_url``|
